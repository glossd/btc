...
func TestPrintFullInfo(t *testing.T) {
	...
	uncWif, err := btcutil.NewWIF(priv, netParams, false)
	if err != nil {
		log.Fatal(err)
	}
	...
	cmpWif, err := btcutil.NewWIF(priv, netParams, true)
	if err != nil {
		log.Fatal(err)
	}
	...
	uncAddr, err := btcutil.NewAddressPubKey(uncPubBytes, netParams)
	if err != nil {
		log.Fatal(err)
	}

	cmpAddr, err := btcutil.NewAddressPubKey(cmpPubBytes, netParams)
	if err != nil {
		log.Fatal(err)
	}
